...
tracing = SanicTracing(
    tracer=MockTracer(), app=app, trace_all_requests=False, exceptions=[RuntimeError,]
)
...
@app.route("/attributes_test")
@tracing.trace("url", "ip", "method")
def decorated_fn_attribute(request):
    return response.text("Attribute Test")
...
async def send_request(request):
    ...
    res = await app.aiohttp_session.get(f"http://{request.host}/decorated_test")
