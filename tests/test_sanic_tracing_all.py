...
from .sanic_app_trace_all import app, test_app, tracing
...
class TestTracing(unittest.TestCase):
    ...
    def test_trace_all(self):
        ...
        def intercept_middleware(request: Request):
            assert tracing.get_span(request)
            assert tracing._current_scopes[str(request)]
        request, response = test_app.get("/test")
        # Scope being close properly
        assert tracing.get_span(request) is None
...
class TestTracingStartSpanCallback(unittest.TestCase):
    def test_simple(self):
        ...
        tracing = SanicTracing(tracer=MockTracer(), app=app, trace_all_requests=True, start_span_cb=start_span_cb)
        request, response = test_app.get("/test")
        assert "OK" in str(response.status_code)
